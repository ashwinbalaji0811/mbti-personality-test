...
$pers = array('ENFJ', 'ENFP','ENTJ', 'ENTP', 'ESFJ', 'ESFP', 'ESTJ', 'ESTP', 'INFJ', 'INFP', 'INTJ', 'INTP', 'ISFJ', 'ISFP', 'ISTJ', 'ISTP');
...
for($x = 0; $x < 16; $x += 1){
    if (!strcmp($pers[$x], $_GET['pers'])){
        $idPerso = $pers[$x];
        $descripPerso = $persDesc[$idPerso];
        $E = $_GET['e'] * 10;
        $S = $_GET['s'] * 5;
        $T = $_GET['t'] * 5;
        $J = $_GET['j'] * 5;
        $linkVar = 'wiki/personality-list/'. strtolower($idPerso);
    }
}

include("indexRes.php");
?>